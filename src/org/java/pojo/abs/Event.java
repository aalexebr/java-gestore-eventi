...
public class Event {
	...
	public void setDate(int year,int month, int day) throws Exception{
		...
		if(expectedDate.isBefore(today)) {
			 throw new Exception("ERROR : Date inserted is expired");
		}
		else {
...
//		Date todayDate = new Date();
		...
		throw new Exception("ERROR : The event date has already passed");
	}
	
	public void cancelReservation () throws Exception{
		LocalDate today = LocalDate.now();
		if(getReservedSeats()<=0) {
			throw new Exception("ERROR : No reservations to cancel");
		}
	...
	public void cancelMultipleReservations(int n ) throws Exception{
		...
		if(getReservedSeats()<=0) {
			...
			throw new Exception("ERROR : number of reserved seats is less than the number you are trying to cancel");
		}
		if(getDate().isAfter(today) || getDate().isEqual(today)) {
			this.reservedSeats -= n;
			return;
